...
class ABIDStrategySeq: public SearchStrategy
{
 public:
    ABIDStrategySeq(): SearchStrategy("ABID Seq", 3) {}
    SearchStrategy* clone() { return new ABIDStrategySeq(); }
 ...
 */
void ABIDStrategySeq::searchBestMove()
{    
    ...
    //fprintf(stderr, "Best move has val %d\n", currentValue);
 ...
 */
int ABIDStrategySeq::alphabeta(int depth, int alpha, int beta)
{
    ...
    depthPhase = true;
    //fprintf(stderr, "PVmove %sat depth %d\n", m.name(), depth);
    while (1) {
	...
	//if ((m.type) == Move::none) fprintf(stderr, "%s\n", m.name());
...
// register ourselve
ABIDStrategySeq abidStrategySeq;
